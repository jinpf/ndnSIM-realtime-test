...
#include "ndn-producer-realtime.h"
#include "ns3/log.h"
...
{
  static TypeId tid = TypeId ("ns3::ndn::ProducerR")
    .SetGroupName ("Ndn")
    .SetParent<App> ()
    .AddConstructor<ProducerR> ()
    .AddAttribute ("Prefix","Prefix, for which realtime producer has the data",
                   StringValue ("/"),
                   MakeNameAccessor (&ProducerR::m_prefix),
                   MakeNameChecker ())
    // .AddAttribute ("Postfix", "Postfix that is added to the output data (e.g., for adding producer-uniqueness)",
    //                StringValue ("/"),
    //                MakeNameAccessor (&ProducerR::m_postfix),
    //                MakeNameChecker ())
    ...
    .AddAttribute ("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&ProducerR::m_freshness),
                   MakeTimeChecker ())
    .AddAttribute ("Signature", "Fake signature, 0 valid signature (default), other values application-specific",
                   UintegerValue (0),
                   MakeUintegerAccessor (&ProducerR::m_signature),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("KeyLocator", "Name to be used for key locator.  If root, then key locator is not used",
                   NameValue (),
                   MakeNameAccessor (&ProducerR::m_keyLocator),
                   MakeNameChecker ())

    .AddAttribute ("Frequency", "Frequency of data packet generate",
                   StringValue ("1.0"),
                   MakeDoubleAccessor (&ProducerR::m_frequency),
...
void
ProducerR::OnInterest (Ptr<const Interest> interest)
{
...
void
ProducerR::SendData(const uint32_t &seq)
{
